...
**Important**: The backend enforces quota surcharge parity at `POST /api/campaigns`. CLI fetches eligible user count from `GET /api/targeting/count` and includes quota surcharge in fee calculation before funding. The backend allows one-tier tolerance on surcharge comparison. If the targeting count endpoint is unavailable, execute mode aborts unless `--allow-fee-uncertain` is passed. See `error-playbook.md` for recovery steps if a `fee_insufficient` error occurs.
